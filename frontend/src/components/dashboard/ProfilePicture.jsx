...
import { readProfilePhoto, writeProfilePhoto } from "@/lib/profile-photo-storage";

const ProfilePicture = ({ name, type, userId, editable = false }) => {
  const initials = name
    .split(" ")
    .map((n) => n[0])
    .join("")
    ...
    if (file) {
      ...
      reader.onload = (ev) => {
        ...
        if (typeof url === "string") {
          setImageUrl(url);
          writeProfilePhoto(type, userId, url);
        }
      };
